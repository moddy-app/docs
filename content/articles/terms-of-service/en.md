# Terms of Service

**Last updated: December 31, 2025**

These Terms of Service (hereinafter "ToS") govern the use of Moddy services, including the Discord bot, website, and all associated services.

## Service Presentation

**Moddy** is a Discord bot offering moderation, server management, customer support, and automation features.

**Service publisher:**  
Moddy  
Contact: hello@moddy.app  
Support: [moddy.app/support](https://moddy.app/support)

**Open Source Project:**  
Moddy is an open source project distributed under the **[Moddy Community License (MCL)](/articles/license/)**. The source code is publicly accessible and can be viewed, modified, and redistributed in accordance with the terms of this license and these ToS.

## Acceptance of Terms

### Scope of acceptance

By using Moddy services (Discord bot, website, API, or any other associated service), you unconditionally accept these ToS as well as our Privacy Policy.

The use of Moddy's source code, whether for consultation, modification, or redistribution, also implies acceptance of these ToS in addition to the CC BY-NC-SA license.

### Legal capacity

Use of our services is reserved for:

- Persons aged at least 13 years (in accordance with Discord's terms)
- Persons with legal capacity to contract
- Minors with parental or legal guardian authorization

By using Moddy, you declare and guarantee that you meet these conditions.

### Implied acceptance

Continued use of our services after any modification of the ToS constitutes acceptance of the new terms.

## Service Description

### Services offered

Moddy offers the following features:

- **Automatic moderation**: detection and sanctioning of inappropriate behavior
- **Ticket system**: customer support and user request management
- **Role management**: automatic role assignment and saving
- **Reminder system**: scheduled personalized notifications
- **Cross-server messages**: message broadcasting across multiple Discord servers
- **Utility commands**: various tools for server management and animation
- **Artificial intelligence**: translation, content generation via OpenAI
- **Moderation history**: complete tracking of sanctions and moderation cases
- **Transcripts**: ticket conversation backups

### Service availability

We strive to keep our services accessible 24/7, but cannot guarantee uninterrupted availability.

Services may be temporarily suspended for:

- Scheduled or emergency maintenance
- Updates and improvements
- Technical issues beyond our control
- Force majeure

We reserve the right to modify, suspend, or discontinue all or part of the services at any time, with or without notice.

### Service evolution

Moddy reserves the right to:

- Add new features
- Modify or remove existing features
- Evolve the interface and user experience

Users will be informed of major changes via our official Discord server and/or by email.

### Technical intervention in servers

Our team reserves the right to intervene in users' Discord servers without prior notice, but with appropriate justification, particularly for:

- Critical bug fixes
- Technical problem resolution
- Emergency intervention in case of major malfunction
- Compliance verification with ToS
- Investigation of reports

These interventions are carried out with respect for user privacy and only to the extent necessary to resolve the identified problem.

## Terms of Use

### User obligations

By using Moddy, you agree to:

- Provide accurate and up-to-date information
- Comply with applicable laws and regulations
- Comply with Discord's terms of use
- Not infringe on third-party rights
- Use the services reasonably and fairly
- Not attempt to circumvent security measures
- Report any malfunction or security vulnerability

### Source code usage

Use of Moddy's source code is subject to the following conditions:

- Compliance with the [Moddy Community License (MCL)](/articles/license/)
- Compliance with these ToS in any use, modification, or redistribution of the code
- Appropriate attribution of Moddy (in source code and in a location accessible to end users)
- Prohibition of commercial use without prior written authorization
- Including a copy of the MCL in any redistribution
- No claiming ownership of the original Software or misrepresenting its origin

### Prohibited uses

It is strictly forbidden to use Moddy for:

- **Illegal content**: distribution of illegal, violent, pornographic content involving minors, discriminatory, defamatory, or privacy-infringing content
- **Harassment**: harassing, threatening, intimidating other users
- **Spam and abuse**: mass sending of unsolicited messages, abusive use of commands
- **Fraud**: identity theft, fraud attempts, phishing
- **Hacking**: unauthorized access attempts, security vulnerability exploitation, DDoS attacks
- **Circumvention**: circumventing sanctions, creating multiple accounts to avoid a ban
- **Resale**: unauthorized resale of Moddy services
- **Abusive automation**: using scripts or bots to automate Moddy usage abusively

### Moderation and sanctions

In case of non-compliance with these ToS, Moddy reserves the right, **with or without notice, without having to justify itself, and without compensation**, to:

- Temporarily suspend access to services
- Permanently ban the user (BLACKLISTED status)
- Remove inappropriate content
- Report illegal behavior to competent authorities
- Terminate Premium subscriptions without refund
- Cut off access to all or part of the services

Sanctions applied depend on the severity of the violation and may be applied at Moddy team's sole discretion.

### Reporting to authorities

In case of serious violation of the law (child pornography, terrorism, death threats, illegal trafficking, etc.), Moddy reserves the right to use collected data to report to competent authorities, in accordance with our legal obligations.

These reports may be made without prior notice or information to the user concerned, in compliance with applicable legislation.

### User content responsibility

You are solely responsible for the content you create, share, or distribute via Moddy, including:

- Saved messages
- Personalized reminders
- Cross-server messages
- Tickets and transcripts
- Any other data generated by your use

Moddy is not responsible for user-created content and acts as a technical host.

## Premium Services and Payments

### Premium subscriptions

Moddy offers Premium subscriptions with advanced features:

- Increased limits (reminders, saved messages, etc.)
- Exclusive features
- Priority support
- Advanced customization

Details of Premium offers are available on our website and may evolve.

### Pricing

Prices are displayed in euros (EUR) including all taxes (VAT included) on our website and via Stripe.

Moddy reserves the right to modify its prices at any time. Price changes:

- Apply immediately to new subscriptions
- Apply at next renewal for existing subscriptions
- Are communicated at least 30 days before their application

### Payment methods

Payments are processed securely by **Stripe**, our PCI-DSS certified payment provider.

Accepted payment methods:

- Bank cards (Visa, Mastercard, American Express)
- Other means offered by Stripe depending on your region

Payments are made:

- Upon initial subscription
- Automatically at each renewal period (monthly or annual)

### Renewal and cancellation

**Automatic renewal:**  
Premium subscriptions are automatically renewed at the end of each period, unless previously cancelled.

**Cancellation:**  
You can cancel your subscription at any time via your customer area on our website or by contacting our support.

Cancellation takes effect at the end of the current period. No refund is made for the current period, except for legal exceptions.

### Right of withdrawal

In accordance with European regulations, you have a period of **14 days** from the subscription to exercise your right of withdrawal, without having to justify a reason.

To exercise this right, contact our support.

The refund will be made within 14 days following the withdrawal request.

**Exception:** If you have expressly requested immediate execution of the service during the withdrawal period, you will not be able to exercise this right or the refund will be proportional to the use.

### Refund policy

**No refund** is granted in the following cases:

- Voluntary cancellation during subscription (outside withdrawal period)
- Ban for ToS violation
- Non-use of services
- Dissatisfaction after the withdrawal period

**Exceptional refunds:**  
Refunds may be granted on a case-by-case basis in case of:

- Prolonged and proven service malfunction
- Billing error
- Exceptional circumstances at Moddy's discretion

## Intellectual Property

### Open Source License

Moddy's source code is distributed under the **[Moddy Community License (MCL) v1.0](/articles/license/)**.

This license allows you to:

- **Use** the Software for personal or non-commercial projects
- **Modify** the Software and create derivative works for personal or non-commercial purposes
- **Redistribute** the Software or derivative works, provided that the conditions below are met

Under the following conditions:

- **Attribution required**: you must credit Moddy appropriately, include a link to the original repository, and indicate if changes were made — this credit must appear both in the source code and in a location accessible to end users
- **Non-association**: the credit does not imply any endorsement, partnership, or affiliation with Moddy
- **Non-Commercial**: you may not use the Software for commercial purposes without prior written authorization
- **License notice**: a copy of the MCL must be included in any redistribution

You are **not** permitted to sell the Software, claim ownership of it, or redistribute it without proper credit.

For the full license text: [docs.moddy.app/articles/license/](/articles/license/)

### Elements excluded from Open Source license

The following are excluded from the Open Source license and remain Moddy's exclusive property:

- The name "Moddy", logos, trademarks, and visual identity graphic elements
- Databases and their content
- API keys and configuration secrets
- Commercial and marketing documentation
- Website-specific editorial content

### Moddy property

All elements not covered by the Open Source license are and remain Moddy's exclusive property.

### Service usage license

Using Moddy services (Discord bot, website) grants you a non-exclusive, non-transferable, revocable, and limited license to:

- Use the services in accordance with these ToS
- Access features for your personal use or for your Discord server

This license does not grant you any ownership rights over the services or their content.

### Restrictions

It is forbidden to:

- Use the name "Moddy" or logos without prior written authorization
- Use the source code for commercial purposes without authorization
- Not comply with the CC BY-NC-SA license attribution conditions
- Extract or reuse a substantial part of the databases

### User content

Regarding content you create via Moddy:

- You retain all your intellectual property rights
- You grant Moddy a worldwide, free, non-exclusive license to host, store, process, and display this content as part of providing the services
- This license ends when you delete the content or terminate your account

## Personal Data Protection

The processing of your personal data is governed by our **Privacy Policy**, accessible at: [moddy.app/privacy](https://moddy.app/privacy)

By using Moddy, you acknowledge having read and accepted this policy.

**Key points:**

- Data collected: Discord identifiers, usage data, payment information (via Stripe)
- Purposes: service provision, improvement, customer support, reporting to authorities in case of serious violation
- GDPR rights: access, rectification, erasure, portability
- Use for reporting: in case of serious violation of the law, your data may be transmitted to competent authorities

For more details, please consult the complete Privacy Policy.

## Limitation of Liability

### Moddy's liability

Moddy undertakes to provide its services with diligence and according to the rules of the art.

However, **Moddy cannot be held liable** for:

- Indirect, incidental, or consequential damages
- Loss of profits, data, revenue, or business opportunities
- Temporary or prolonged service interruption
- Bugs, errors, or technical malfunctions
- Actions of third parties (hackers, DDoS attacks)
- Inappropriate use of services by users
- Content created or shared by users
- Data deletion following legitimate ban
- Consequences of technical intervention in users' servers

### Force majeure

Moddy cannot be held liable for non-performance of its obligations in case of force majeure, including:

- Natural disasters
- Wars, attacks, riots
- General strikes
- Telecommunications network failure
- Legislative or regulatory changes
- Large-scale cyberattacks

### Third-party services

Moddy uses third-party services (Discord, Railway, Stripe, OpenAI, etc.). We are not responsible for:

- Malfunctions of these services
- Their privacy policies or terms of use
- Modifications or interruptions of these services

You are invited to consult the terms of use of these third-party platforms.

### Legal limitation

To the extent permitted by applicable law, Moddy's total liability to you may not exceed the total amount of sums actually paid by you over the 12 months preceding the event giving rise to liability.

## Warranties and Representations

### Services provided 'as is'

Moddy services are provided "as is" and "as available", without warranty of any kind, express or implied, including:

- Absence of interruption or errors
- Accuracy or reliability of results
- Correction of defects
- Compatibility with your equipment

### No guarantee of results

Moddy is a tool facilitating Discord server management, but does not guarantee:

- 100% effective automatic moderation
- Total absence of spam or inappropriate behavior
- Compliance with specific requirements of each server

The server administrator remains responsible for moderation and compliance with Discord rules.

## Data and Backups

### Data backup

Moddy performs regular backups of its databases to ensure service continuity.

However, **you are responsible for backing up your own important data**.

### Data loss

Moddy cannot be held liable for data loss following:

- Voluntary or accidental deletion by the user
- Technical malfunction
- Ban for ToS violation
- Service closure

It is recommended to regularly export your important data (transcripts, role lists, etc.).

### Data retention

Data is retained indefinitely until:

- Manual deletion by our technical team
- Explicit deletion request on your part (GDPR right to erasure)

See the Privacy Policy for more details.

## ToS Modifications

### Right to modify

Moddy reserves the right to modify these ToS at any time to reflect:

- Evolution of our services
- Legislative or regulatory changes
- Improvement of our practices

### Notification of modifications

Any substantial modification will be:

- Published on this page with an updated date
- Notified to users via our official Discord server and/or by email
- Effective immediately for new users
- Effective after a reasonable notice period (generally 30 days) for existing users

### Acceptance of modifications

By continuing to use Moddy after the modifications take effect, you accept the new ToS.

If you refuse the modifications, you must stop using the services.

## Termination

### Termination by user

You can stop using Moddy at any time by:

- Removing the bot from your Discord server
- Deactivating your account on our website
- Canceling your Premium subscription

### Termination by Moddy

Moddy reserves the right to suspend or terminate your access to services, **with or without notice, without having to justify itself**, in case of:

- Violation of these ToS
- Fraudulent or abusive behavior
- Non-payment of a Premium subscription
- Prolonged inactivity (at our discretion)
- Definitive service closure
- Any other reason deemed legitimate by Moddy

### Consequences of termination

In case of termination:

- Your access to Premium services is immediately interrupted
- Your data may be deleted in accordance with our retention policy
- No refund is made (except legal obligations)
- ToS clauses that, by nature, must survive (intellectual property, limitation of liability) remain in effect

## General Provisions

### Entire agreement

These ToS, together with the Privacy Policy, constitute the entire agreement between you and Moddy regarding the use of the services.

### Partial invalidity

If a provision of these ToS is deemed invalid or unenforceable by a competent court, the other provisions will remain fully in effect.

The invalid provision will be replaced by a valid provision as close as possible to the original intent.

### No waiver

Moddy's failure to exercise a right or provision of these ToS does not constitute a waiver of that right or provision.

### Assignment

You may not assign or transfer your rights or obligations under these ToS without Moddy's prior written consent.

Moddy may freely assign or transfer its rights and obligations, particularly in case of merger, acquisition, or asset sale.

### Language

These ToS are written in English. In case of translation, only the English version is authoritative.

## Applicable Law and Jurisdiction

### Applicable law

These ToS are governed and interpreted in accordance with **French law**, with the exception of its conflict of laws rules.

For consumer users residing in the European Union, nothing in these ToS affects your rights as a consumer under the legislation of your country of residence.

### Dispute resolution

**Prior mediation:**  
In case of a dispute relating to the interpretation or execution of these ToS, we encourage you to contact us first to try to find an amicable solution via our support.

**Consumer mediation (for EU consumers):**  
In accordance with articles L.612-1 et seq. of the Consumer Code, you have the right to use a consumer mediator free of charge in case of a dispute.

The contact details of the competent mediator will be provided upon request.

**Competent jurisdiction:**  
Failing an amicable solution, any dispute will be submitted to the competent French courts, according to applicable territorial jurisdiction rules.

For consumers residing in the European Union, you may also seize the courts of your place of residence.

**Online Dispute Resolution (ODR) platform:**  
The European Commission provides an online dispute resolution platform accessible at: [https://ec.europa.eu/consumers/odr](https://ec.europa.eu/consumers/odr)

## Contact

**For any questions regarding these ToS, Moddy services, legal requests, or exercising your rights:**

- **Email**: hello@moddy.app
- **Support** (recommended): [moddy.app/support](https://moddy.app/support)

Our support can handle all your requests, including legal requests.

**Official Discord server:**  
Join our server for announcements, community support, and news.

---

> **By using Moddy services (Discord bot, website, and associated services) or by using Moddy's source code, you acknowledge having read, understood, and accepted all of these Terms of Service as well as the [Moddy Community License (MCL)](/articles/license/).**

**Effective date: December 31, 2025**
